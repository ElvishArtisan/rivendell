...
RecordCut::RecordCut(RDCart *cart,QString cut,AudioControls *cntls,
		     QWidget *parent)
  : QDialog(parent,"",true)
{
  bool valid;
  ...
  rec_evergreen_box->setEnabled(rec_controls->use_dayparting_box->isChecked());
  ...
  rec_evergreen_label->
    setEnabled(rec_controls->use_dayparting_box->isChecked());
  ...
  rec_evergreen_box->setEnabled(allow_modification&&rec_controls->use_dayparting_box->isChecked());
  if(!allow_modification) {
  ...
  for(int i=0;i<7;i++) {
    rec_weekpart_button[i]->setEnabled(allow_modification&&rec_controls->use_dayparting_box->isChecked());
  }
  rec_record_button->
...
{
  state=state||(!rec_controls->use_dayparting_box->isChecked());
