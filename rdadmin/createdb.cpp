...
bool InsertRDAirPlayHotkey(const QString &station,const QString &label,int id)
{
...
bool UpdateRDAirplayHotkeys(QString station)
{
  bool ret=true;
  QString sql;

  for(int i=0;i<7;i++) {
    ret=ret&&InsertRDAirPlayHotkey(station,QObject::tr("Start Line")+
				  QString().sprintf(" %d",i+1),3*i+1);
    ret=ret&&InsertRDAirPlayHotkey(station,QObject::tr("Stop Line")+
				  QString().sprintf(" %d",i+1),3*i+2);
    ret=ret&&InsertRDAirPlayHotkey(station,QObject::tr("Pause Line")+
				  QString().sprintf(" %d",i+1),3*i+3);
  }
  ret=ret&&InsertRDAirPlayHotkey(station,QObject::tr("Add"),22);
  ret=ret&&InsertRDAirPlayHotkey(station,QObject::tr("Delete"),23);
  ret=ret&&InsertRDAirPlayHotkey(station,QObject::tr("Copy"),24);
  ret=ret&&InsertRDAirPlayHotkey(station,QObject::tr("Move"),25);
  ret=ret&&InsertRDAirPlayHotkey(station,QObject::tr("Sound Panel"),26);
  ret=ret&&InsertRDAirPlayHotkey(station,QObject::tr("Main Log"),27);
  ret=ret&&InsertRDAirPlayHotkey(station,QObject::tr("Aux Log 1"),28);
  ret=ret&&InsertRDAirPlayHotkey(station,QObject::tr("Aux Log 2"),29);
...
bool InitDb(QString station_name)
{
  ...
  sql=QString("insert into USERS (")+
    ...
    "ADMIN_CONFIG_PRIV) "+
    "values ("+
    "\""+RDEscapeString(RDA_LOGIN_NAME)+"\","+
    "PASSWORD(\""+RDEscapeString(RDA_PASSWORD)+"\"),"+
    "\""+RDEscapeString(RDA_FULLNAME)+"\","+
    "\""+RDEscapeString(RDA_DESCRIPTION)+"\","+
    "\"Y\","+
    "\"Y\")";
  if(!RunQuery(sql)) {
    return false;
  ...
  //
  struct hostent *hostent=gethostbyname(station_name);
  if(hostent==NULL) {
  ...
  sql=QString("insert into CART set ")+
    ...
    "GROUP_NAME=\"TEST\","+
    "TITLE=\"Test Tone\","+
    "ARTIST=\"RivendellRadio Automation\","+
    "CUT_QUANTITY=1,"+
    "FORCED_LENGTH=10000,"+
  ...
  sql=QString("insert into CUTS set ")+
    ...
    "OUTCUE=\"[tone]\","+
    "CODING_FORMAT=0,"+
    QString().sprintf("SAMPLE_RATE=%d,",RD_DEFAULT_SAMPLE_RATE)+
    "BIT_RATE=0,"+
    "CHANNELS=2,"+
    "LENGTH=10000,"+
    "ORIGIN_DATETIME=now(),"+
